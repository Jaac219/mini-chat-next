...
import Messages from "@/components/ChannelSms";
...
export default function Server({params}) {
  ...
  const [categories, loading, error] = useFetchCategories({serverId})
  
  const channels = categories.map((c) => c.channels).flat()
  
  let selectedChannel = 
    channels.find((channel) => channel._id == params[1]) ?? channels[0]
  ...
  return (
    <>
            ...
            <div key={category._id}>
              {category.label && (
                ...
                >
                  <Icons.Arrow
                    className={`${
                      closedCategories.includes(category._id)
                        ? '-rotate-90'
                        : ''
      ...
      <div className='flex flex-col flex-1 flex-shrink min-w-0 bg-gray-700'>
        ...
        {selectedChannel && <Messages channelId={selectedChannel._id} />}    
      </div>
    </>
  )
}
